...
import { getBrief, updateBrief } from '../../../utils/functions';

const EditBrief = () => {
    const { id } = useParams();
    const { register, handleSubmit, setValue } = useForm();
    const [brief, setBrief] = useState(null);
    const { showAlertMessage, loading, setOpenBackDrop } = useContext(AppContext);
    ...
    const onSubmit = async (data) => {
        const result = await updateBrief(id, data);

        if (result) {
            showAlertMessage('Бриф успішно оновлено');
            navigate('/panel/briefs');
        } else {
            showAlertMessage('Помилка оновлення брифу', 'error');
        }
    }
    ...
    useEffect(() => {
        if(brief){
            setValue('lastName', brief.lastName);
            setValue('firstName', brief.firstName);
            setValue('cellphone', brief.cellphone);
            setValue('supportInfo', brief.supportInfo);
            setValue('projectName', brief.projectName);
            setValue('projectDescription', brief.projectDescription);
            setValue('problems', brief.problems);
            setValue('expectation', brief.expectation);
            setValue('audience', brief.audience);
            setValue('audienceInterests', brief.audienceInterests);
            setValue('keyMessages', brief.keyMessages);
            setValue('callToAction', brief.callToAction);
            setValue('competitors', brief.competitors);
            setValue('likeWebSites', brief.likeWebSites);
            setValue('dislikeWebSites', brief.dislikeWebSites);
            setValue('designStyle', brief.designStyle);
            setValue('brand', brief.brand);
            setValue('brandInfo', brief.brandInfo);
            setValue('style', brief.style);
            setValue('stylePage', brief.stylePage);
            setValue('likeSites', brief.likeSites);
            setValue('featuresWebSite', brief.featuresWebSite);
            setValue('structure', brief.structure);
            setValue('structureInfo', brief.structureInfo);
            setValue('chaptersSite', brief.chaptersSite);
            setValue('contentPages', brief.contentPages);
            setValue('adaptive', brief.adaptive);
            setValue('adaptiveInfo', brief.adaptiveInfo);
            setValue('paymentSystem', brief.paymentSystem);
            setValue('paymentSystemInfo', brief.paymentSystemInfo);
            setValue('featuresWebSite', brief.featuresWebSite);
            setValue('deadline', brief.deadline);
            setValue('budget', brief.budget);
            setValue('expectedResults', brief.expectedResults);
            setValue('successSite', brief.successSite);
            setValue('managerSite', brief.managerSite);
            setValue('managerSiteInfo', brief.managerSiteInfo);
            setValue('contentUs', brief.contentUs);
            setValue('contentUsInfo', brief.contentUsInfo);
            setValue('additionalInfo', brief.additionalInfo);
        }
    }, [brief]);
                    ...
                    <div className="container mt-5">
                        <div className="row justify-content-center">
                            <div className="col-md-12 text-light pb-5">
                                <h1 className="p-5">Бриф на розробку веб-сайту (Редагування)</h1>
                                <form onSubmit={handleSubmit(onSubmit)}>
                                    ...
                                    <div className="d-flex justify-content-center">
                                        <button type="submit" className="btn btn-outline-info">Оновити</button>
                                    </div>
                                </form>
                            </div>
                        </div>
